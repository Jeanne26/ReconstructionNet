...
fig_b, axes_b = plt.subplots(TOP_K, 5, figsize=(20, 3.5 * TOP_K))
if TOP_K == 1:
...
for i, idx in enumerate(top_distributional):
    ...
    colors = ['#d32f2f' if c == pred else '#1976d2' if c == label else '#bdbdbd'
              for c in range(num_classes)]
    axes_b[i, 1].barh(range(num_classes), probs, color=colors)
    axes_b[i, 1].set_yticks(range(num_classes))
    axes_b[i, 1].set_xlabel('Probability')
    axes_b[i, 1].set_title(f"Distribution")
    axes_b[i, 1].set_xlim(0, 1)
    ...
    axes_b[i, 2].imshow(img.reshape(img_size, img_size), cmap='gray')
    axes_b[i, 2].imshow(mask.reshape(img_size, img_size), cmap='Reds', alpha=0.6, vmin=0, vmax=1)
    axes_b[i, 2].set_title(f"Explanation")
    axes_b[i, 2].axis('off')

 
    axes_b[i, 3].imshow(all_r_list[idx][label].reshape(img_size, img_size), cmap='gray')
    axes_b[i, 3].set_title(f"Recons (ae {label})")
    axes_b[i, 4].axis('off')

    axes_b[i, 4].imshow(all_r_list[idx][pred].reshape(img_size, img_size), cmap='gray')
    axes_b[i, 4].set_title(f"Recons (ae {pred})")
    axes_b[i, 4].axis('off')
...
fig_c, axes_c = plt.subplots(TOP_K, 4, figsize=(16, 3.5 * TOP_K))
if TOP_K == 1:
